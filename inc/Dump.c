...
// The software simply reads TIMG12->COUNTERREGS.CTR to get the current time in bus cycles.
  if(DebugCnt < MAXBUF){
    ...
    uint32_t thingy = TIMG12->COUNTERREGS.CTR;
    TimeBuffer[DebugCnt] = thingy;
    DebugCnt++;
    return 1;
  }
  return 0; // success
...
//    if zero, the signal is considered low.
  ...
  uint32_t prev = (DataBuffer[0] & mask) ? 1:0;
  uint32_t current = 0;
  uint32_t riseTime = 0;
  uint32_t periods = 0;
  uint32_t sum = 0;
  bool firstRise = false;

  for(uint32_t i = 1; i<DebugCnt; i++)
  {
    current = (DataBuffer[i] & mask) ? 1:0;
    if(prev==0 && current==1){
      if(!firstRise)
      {
        riseTime = TimeBuffer[i];
        firstRise = true;
      }
      else{
        sum += riseTime - TimeBuffer[i];
        periods++;
        riseTime = TimeBuffer[i];
      }
    }
  ...
  return sum / periods; // average period
}
