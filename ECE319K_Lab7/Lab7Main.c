...
void OutFix(uint32_t position){
  // Split into integer and fractional parts
  uint32_t wholeCm = position / 1000; 
  uint32_t fracCm = position % 1000; 
  
  uint32_t hundreds = fracCm / 100; 
  uint32_t tens = (fracCm % 100) / 10; 
  uint32_t ones = fracCm % 10; 
  
  // Move LCD cursor to a fixed position
  ST7735_SetCursor(0, 1);
  
  // Output digit by digit: "1.234 cm"
  ST7735_OutChar('0' + wholeCm);
  ST7735_OutChar('.');
  ST7735_OutChar('0' + hundreds);
  ST7735_OutChar('0' + tens);
  ST7735_OutChar('0' + ones);
  ST7735_OutString(" cm");
}
